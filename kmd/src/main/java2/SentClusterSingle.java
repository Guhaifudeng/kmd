 ...
 */
public class SentClusterSingle {
    public static void main(String[] args) throws Exception{
        ...
        MyTimeClock mt = new MyTimeClock("---S400---");
        mt.tt();
        //引入sent map
...
//
        ...
        for(Cluster c1:clusters_set){
            ...
            for(Cluster c2:clusters_set){
                if(i<=j){break;}
                Pair<Double,Integer> SEC_pair = chameleonTool.calSEC(c1,c2);
                Double SEC = SEC_pair.getFirst() / SEC_pair.getSecond();
                Double RC = chameleonTool.calRC(c1,c2,SEC_pair);
                Double RI = chameleonTool.calRI(c1,c2,SEC_pair);
        ...
        while(!clusters_set.isEmpty() && !clusters_priQueue.isEmpty()){
            ...
            for(Cluster cluster:clusters_set){
                Pair<Double,Integer> t_SEC_pair = chameleonTool.calSEC(c1,cluster);
                Double t_SEC = t_SEC_pair.getFirst() / t_SEC_pair.getSecond();
                Double t_RC = chameleonTool.calRC(c1,cluster,t_SEC_pair);
                Double t_RI = chameleonTool.calRI(c1,cluster,t_SEC_pair);
