...
public class Chameleon2 {
    public static void main(String[] args) throws Exception{
        ...
        ComparetorERC comparetorByERC = new ComparetorERC();
        Queue<Cluster> clusters_set = new PriorityQueue <>(set_s.size(),comparetorByERC);
        ...
        Double minESC = 0.6;
        Double alpha1 = 1.0;
        Double alpha2 = 2.0;//RI*RC^alpha
        String sort_para = "opt";
...
//                String.valueOf(minMetric).substring(0,3)+"_step2.txt";
        out2 += "_minSEC_"+minESC+"_knn_"+knn+"_minRC_"+minRC+"_minRI_"+minRI+"_minMetirc_"+ minMetric+"_"+sort_para+"_step2.txt";
...
//                String.valueOf(minMetric).substring(0,3)+"_log.txt";
        log_file += "_minSEC_"+minESC+"_knn_"+knn+"_minRC_"+minRC+"_minRI_"+minRI+"_minMetirc_"+
                minMetric+"_"+sort_para+"_log.txt";
        log2_file += "_minSEC_"+minESC+"_knn_"+knn+"_minRC_"+minRC+"_minRI_"+minRI+"_minMetirc_"+
            ...
            while(iterator.hasNext()){
                ...
                if(tmp.isIs_merged()){
                    if(cluster.isIs_merged()){
                        if(opt > max_opt && RC>minRC && RI>minRI && ESC>minESC){
                            max_opt = opt;
                            max_cluster = tmp;
                            max_RC = RC;
                            max_RI = RI;
                        ...
                        if(RC > minRC && ESC >minESC && ESC > max_ESC){
                            max_opt = opt;
                            max_cluster = tmp;
                            max_RC = RC;
                            max_RI = RI;
...
//                log.println("-opt: "+max_opt+" -RC:"+max_RC+" -RI:"+max_RI+" now_size: "+cluster.getPointSize()+" max_size: "+max_cluster.getPointSize());
            if(max_cluster!=null&&(
                    ...
                    (!max_cluster.isIs_merged()&&cluster.isIs_merged()&& max_RC >= minRC&& max_ESC > minESC )||
                    (!cluster.isIs_merged()&& max_cluster.isIs_merged()&&max_RC >= minRC && max_ESC >minESC )||
                ...
                if(notbalance)
                     cluster = chameleonTool.mergeTwoClustersToOne(cluster,max_cluster,max_opt,max_RI,max_RC,notbalance);
                else
        ...
        while(clusterIterator2.hasNext()){
            ...
            pw2.print("\n");
            pw2.print(cluster2.isIs_merged()+"\n");
...
class ComparetorERC implements Comparator {
